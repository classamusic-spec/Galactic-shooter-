...
function nordicMaterials(ctx: BodyBuildContext): void {
  const b = ctx.builder;
  // The library authors its albedos physically — frost-iron sits near 0.10-0.14
  // linear, which is correct for iron and far too dark once a vertex tint is
  // multiplied on top of it. `THREE.Color` is not clamped to 1 and the multiply
  // happens in linear space, so a >1 diffuse lifts a recipe to the value this
...
function buildHuscarl(ctx: BodyBuildContext): BuiltBody {
  ...
  rig.chain('cloak', ['c0', 'c1', 'c2'], [0.34, 0.3, 0.18], {
    parent: 'spine.chest',
    origin: v(0, 0.14, 0.26),
    direction: v(0, -0.9, 0.44).normalize(),
    pole: UP,
    kind: 'tail',
    // Small capture radii on purpose: a wide one here steals the leg and skirt
    // vertices and drags the whole lower body into a flipper behind the unit.
    capture: [0.2, 0.19, 0.16],
  });
  ...
  // Half-cape at the back. Geometry on every tail chain is not optional: an
  // empty chain still claims skin weights, and a claimed-but-invisible chain
...
function buildJarl(ctx: BodyBuildContext): BuiltBody {
  ...
  rig.chain('cloak', ['c0', 'c1', 'c2'], [0.46, 0.42, 0.24], {
    parent: 'spine.chest',
    origin: v(0, 0.2, 0.36),
    direction: v(0, -0.9, 0.42).normalize(),
    pole: UP,
    kind: 'tail',
    capture: [0.26, 0.24, 0.2],
  });
